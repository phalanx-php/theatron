...
final class DevToolsOverlay implements Component
{
    ...
    public function __invoke(RenderContext $ctx): Renderable
    {
        ...
        return column(
            self::renderTabBar($ctx->theme, $tab),
            panel(
                '',
                self::renderTabContent($ctx->theme, $tab, $this->store, $this->mountSystem, $this->registry),
                style: Style::of(size: Size::fill(), border: Border::Rounded, color: Color::indexed(208)),
            ),
        );
    }

    private static function renderTabBar(Theme $theme, int $activeTab): Renderable
    {
        ...
        return text(Line::from(...$spans), Style::of(size: Size::fill()));
    }
    ...
    private static function renderTabContent(
        ...
        return match ($tab) {
            DevToolsTab::Signals->value => self::renderSignalsTab($theme, $registry),
            DevToolsTab::Tree->value    => self::renderTreeTab($theme, $mountSystem),
            DevToolsTab::Store->value   => self::renderStoreTab($theme, $store),
            default                     => self::renderMetricsTab($theme, $store, $mountSystem, $registry),
        };
    }
    ...
    private static function renderSignalsTab(Theme $theme, SignalRegistry $registry): Renderable
    {
    ...
    private static function renderTreeTab(Theme $theme, MountSystem $mountSystem): Renderable
    {
        $nodes = [];
    ...
    private static function renderStoreTab(Theme $theme, AppStore $store): Renderable
    {
        $conv     = $store->conversation;
        $agents   = $store->agents;
        $activity = $store->activity;
